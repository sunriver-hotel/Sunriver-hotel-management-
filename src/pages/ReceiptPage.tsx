...
const ReceiptTemplate: React.FC<{ 
    ...
    const { allRooms, t, language } = useContext(AppContext);
    if (bookings.length === 0) return null;

    const mainBooking = bookings[0];
    const locale = language === 'en' ? 'en-GB' : 'th-TH';
    const receiptDate = new Date().toLocaleDateString(locale, { year: 'numeric', month: 'long', day: 'numeric' });
    const formattedPaymentDate = new Date(paymentDate).toLocaleDateString(locale, { year: 'numeric', month: 'long', day: 'numeric' });
                        ...
                        <tr key={index} className="border-b border-gray-200">
                            <td className="py-2">
                                ...
                                <span className="text-xs text-gray-500">
                                    Check-in: {new Date(item.checkIn).toLocaleDateString(locale)} - 
                                    Check-out: {new Date(item.checkOut).toLocaleDateString(locale)}
                                </span>
                            </td>
    ...
    return (
        <>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full">
                <div className="lg:col-span-1 bg-white p-6 rounded-xl shadow-lg flex flex-col no-print">
                    <div>
                        <input 
                            type="text"
                    ...
                    <div className="mb-4 border-t pt-4">
                        <h3 className="font-semibold mb-2 text-gray-700">{t('payment_information')}</h3>
                        <div className="space-y-3">
                            <div>
                                ...
                                <select 
                                    ...
                                    className="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-amber-500 focus:border-amber-500 sm:text-sm disabled:bg-gray-100"
                    ...
                    <div className="flex-1 overflow-y-auto border-t pt-4">
                        <h3 className="font-semibold mb-2 text-gray-700">{t('recent_bookings')}</h3>
                        <ul className="space-y-2">
                            {filteredBookings.map(b => (
                                <li key={b.id} className="flex items-center justify-between p-3 border rounded-md hover:bg-gray-50 transition-colors">
                                    <label className="flex items-center space-x-3 flex-grow cursor-pointer">
                                        <input 
                                          ...
                                          className="form-checkbox h-5 w-5 text-[#e6c872] rounded focus:ring-[#e6c872] border-gray-300"
                                        />
                                        <div>
                                    ...
                                    <button
                                        ...
                                        className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-100 rounded-full transition-colors ml-2 shrink-0"
                ...
                <div className="lg:col-span-2 bg-slate-200 p-4 rounded-lg shadow-inner overflow-y-auto">
                     <div className="flex justify-end mb-4 no-print">
                        <button 
                            onClick={handlePrint} 
                            disabled={selectedBookings.length === 0}
                            className="px-4 py-2 bg-[#e6c872] text-white rounded-lg font-semibold hover:bg-amber-500 disabled:bg-gray-400 transition-colors"
                        >
                            {t('print_receipt')}
                        </button>
                    </div>
                    {selectedBookings.length > 0 ? (
                        ...
                        <div className="flex items-center justify-center h-full text-gray-500">
                            <p className="text-lg">{t('no_booking_selected')}</p>
                        </div>
                    )}
                </div>
            </div>
